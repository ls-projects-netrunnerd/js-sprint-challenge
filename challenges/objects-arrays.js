...
const tyrannosaurus = {
  ...
  roar: _ => 'RAWERSRARARWERSARARARRRR!'
};
...
for (let g in graduates) {
  let grad = graduates[g];
...
for (let g in graduates) {
  let university = graduates[g].university;

  if (university.includes('Uni')) uni.push(university);
}
...
const largerPopulation = zooAnimals.filter(a => a.population < 5 ? a : 0);
...
const populationTotal = zooAnimals.reduce((total, a) => total + a.population, 0);
